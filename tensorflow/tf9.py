...
y_train = np.array([7.6, 8.5, 10.4, 21.0, 25.3])
...
# 第四个参数num_epoches为epoch的次数，将训练集的所有数据都训练一遍为1次epoch
# 第五个参数为shuffle为去训练数据是顺序取还是随机取
train_input_fn = tf.estimator.inputs.numpy_input_fn(
    {"x": x_train}, y_train, batch_size=2, num_epochs=None, shuffle=False
)
...
eval_input_fn = tf.estimator.inputs.numpy_input_fn(
    {"x": x_eavl}, y_eval, batch_size=2, num_epochs=1000, shuffle=False
)
